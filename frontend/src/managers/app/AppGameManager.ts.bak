...
export class AppGameManager {
  ...
  constructor(app: any) {
    this.app = app;
    logger.info('AppGameManager', '🏗️ AppGameManager initialized');
  }
  ...
  async startGame(): Promise<void> {
    ...
    try {
      ...
      if (!gameCoordinator) {
        logger.info('AppGameManager', 'Creating GameCoordinator instance');
      ...
      logger.info('AppGameManager', '✅ Game started successfully');
      showToast('Game started!', 'success');
      await this.app.router.navigateToScreen('game');
    } catch (error) {
      logger.error('AppGameManager', '💥 Error starting game:', error);
  ...
  async stopGame(): Promise<void> {
    logger.info('AppGameManager', '⏹️ Stopping game...');

    try {
      if (!gameCoordinator) {
        logger.warn('AppGameManager', 'GameCoordinator not available for stop');
      ...
      logger.info('AppGameManager', '✅ Game stopped successfully');
      showToast('Game stopped', 'info');
      await this.app.router.navigateToScreen('main-menu');
    } catch (error) {
      logger.error('AppGameManager', '💥 Error stopping game:', error);
   ...
   */
  pauseGame(): void {
    logger.info('AppGameManager', '⏸️ Toggling game pause...');

    try {
      if (!gameCoordinator) {
        logger.warn('AppGameManager', 'GameCoordinator not available for pause');
        showToast('Game system not initialized', 'error');
        return;
      }
      ...
      if (isPaused) {
        ...
        logger.info('AppGameManager', '▶️ Game resumed');
        showToast('Game resumed', 'info');
      } else {
        gameCoordinator.pauseGame(); // This actually toggles pause/resume
        logger.info('AppGameManager', '⏸️ Game paused');
        showToast('Game paused', 'info');
      }
    } catch (error) {
      logger.error('AppGameManager', '💥 Error toggling game pause:', error);
