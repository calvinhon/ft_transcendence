...
export class AppScreenManager {
  ...
  constructor() {
    logger.info('AppScreenManager', '🏗️ AppScreenManager initialized');
  ...
  private cacheScreenElements(): void {
    ...
    logger.info('AppScreenManager', 'Screen elements cached');
  }
  ...
  showScreen(screenName: string): void {
    ...
    if (targetScreen) {
      ...
      logger.info('AppScreenManager', `Screen shown: ${screenName}`);
    }

    // Set body data attribute for CSS targeting
    document.body.setAttribute('data-current-screen', screenName);

    logger.info('AppScreenManager', `Screen navigation complete: ${screenName}`);
  }
