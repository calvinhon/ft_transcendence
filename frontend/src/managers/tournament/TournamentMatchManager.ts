...
export class TournamentMatchManager {
  ...
  public async playMatchFromCard(tournamentId: number, matchId: number): Promise<void> {
    ...
    if (!matchCard) {
      logger.error('TournamentMatchManager', 'Match card not found');
      return;
    }
    ...
    if (!leftPlayer || !rightPlayer) {
      logger.error('TournamentMatchManager', 'Player elements not found');
      return;
    }
  ...
  public async recordMatchResult(tournamentId: number, matchId: number, winnerId: number, player1Score: number, player2Score: number): Promise<any> {
    ...
    try {
      ...
      logger.error('TournamentMatchManager', 'Record match result error', error);
  ...
  public async recordOnBlockchain(tournamentId: number, winnerId: number): Promise<void> {
    try {
      ...
      logger.error('TournamentMatchManager', 'Blockchain recording error', error);
