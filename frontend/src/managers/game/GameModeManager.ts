...
export class GameModeManager {
  ...
  public async startBotMatch(): Promise<void> {
    ...
    try {
      ...
      logger.error('GameModeManager', 'Failed to start bot match', error);
  ...
  public async startTournamentMatch(): Promise<void> {
    ...
    if (!app || !app.currentTournamentMatch) {
      logger.error('GameModeManager', 'No tournament match data found!');
      return;
    }
  ...
  private async startArcadeMatchWithSettings(): Promise<void> {
    ...
    try {
      ...
      logger.error('GameModeManager', 'Failed to start arcade match', error);
      this.hideWaitingMessage();
  ...
  private async startCampaignMatch(): Promise<void> {
    ...
    try {
      ...
      logger.error('GameModeManager', 'Failed to start campaign match', error);
      this.hideWaitingMessage();
