...
export class PerformanceManager {
  ...
  public initialize(): void {
    ...
    logger.info('PerformanceManager', 'Performance Manager initialized');
  }
  ...
  private handleManagerInitMetric(metric: string, value: any): void {
    ...
    if (value.initTime > 50) { // More than 50ms
      logger.warn('PerformanceManager', `Slow manager initialization: ${managerName}`, {
  ...
  private handleLowFps(metric: any): void {
    logger.warn('PerformanceManager', 'Low FPS detected, analyzing causes...', metric);
    ...
    if (memoryUsage && memoryUsage.value) {
      ...
      if (usagePercent > 90) {
        logger.error('PerformanceManager', 'High memory usage may be causing low FPS', {
  ...
  private logManagerInitSummary(): void {
    ...
    logger.info('PerformanceManager', 'Manager initialization summary', {
  ...
  private analyzeGamePerformanceTrend(): void {
    ...
    if (avgFps < 50 && minFps < 30) {
      logger.warn('PerformanceManager', 'Consistent low game performance detected', {
  ...
  private analyzeApiPerformanceTrend(endpoint: string, history: Array<{duration: number, timestamp: number}>): void {
    ...
    if (avgDuration > 1000 && history.length >= 10) { // 1 second average over 10+ calls
      logger.warn('PerformanceManager', `Consistently slow API endpoint: ${endpoint}`, {
  ...
  private handleSlowApiCall(metric: any): void {
    logger.error('PerformanceManager', 'Very slow API call detected', {
  ...
  private async checkApiHealth(endpoint: string): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        logger.error('PerformanceManager', `API endpoint health check failed: ${endpoint}`, {
          status: response.status
        });
      }
    } catch (error) {
      logger.error('PerformanceManager', `API endpoint unreachable: ${endpoint}`, error);
    }
  }
  ...
  private suggestPerformanceOptimizations(metric: any): void {
    ...
    if (suggestions.length > 0) {
      logger.info('PerformanceManager', 'Performance optimization suggestions', { suggestions });
    }
  }
