...
export class SettingsPage extends AbstractComponent {
  ...
  private renderContent(): string {
    ...
    return `
            <div class="w-full max-w-2xl space-y-8">
                ...
                ${
                  this.error
                    ? `<div class="bg-red-900/20 border border-red-500 text-red-500 p-3 text-center">${this.error}</div>`
                    : ""
                }
                ${
                  this.successMsg
                    ? `<div class="bg-green-900/20 border border-green-500 text-green-500 p-3 text-center">${this.successMsg}</div>`
                    : ""
                }
                    ...
                    <div class="flex items-start gap-6">
                        <div class="w-24 h-24 border-2 border-accent rounded-full overflow-hidden bg-black flex-shrink-0">
                             ${
                               p.avatarUrl
                                 ...
                                 : `<div class="w-full h-full flex items-center justify-center text-2xl text-gray-600">${p.username
                                     .charAt(0)
                                     .toUpperCase()}</div>`
                             }
                        </div>
                        <div class="flex-1 space-y-4">
                            <div>
                                ...
                                <input type="text" id="input-avatar" value="${
                                  p.avatarUrl || ""
                                }" 
                ...
                <div class="border border-white/20 p-6 bg-white/5">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <div>
                            ...
                            <input type="text" id="input-displayname" value="${
                              p.username
                            }" maxlength="16"
                                class="w-full bg-black border border-white/30 p-2 text-white focus:border-accent outline-none font-mono text-sm">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" id="input-country" value="${
                              p.country || ""
                            }" 
                                class="w-full bg-black border border-white/30 p-2 text-white focus:border-accent outline-none font-mono text-sm"
                                placeholder="Unknown">
                ...
                <!-- Display Settings -->
                <div class="border border-white/20 p-6 bg-white/5">
                    <h2 class="text-accent mb-4 border-b border-white/10 pb-2">DISPLAY SETTINGS</h2>
                    ${this.render3DModeSection()}
                    <div class="mt-6 pt-4 border-t border-white/10">
                        ${this.renderPostProcessingSection()}
                    </div>
                </div>
  ...
  private renderPostProcessingSection(): string {
    const webglService = WebGLService.getInstance();
    const isPostProcessingEnabled = webglService.isPostProcessingEnabled();

    return `
            <div class="flex items-center justify-between">
                <div>
                    <div class="text-sm">POST-PROCESSING EFFECTS</div>
                    <div class="text-xs text-gray-500">SSAO, lens distortion, fog</div>
                </div>
                <button id="toggle-post-processing" 
                    class="px-4 py-2 border ${
                    ...
                    transition-all font-mono text-sm flex items-center gap-2">
                    <i class="fas ${
                      isPostProcessingEnabled ? "fa-sparkles" : "fa-square"
                    }"></i>
            ...
            <div class="mt-3 text-xs text-gray-600">
                <i class="fas fa-info-circle mr-1"></i>
                Disable if experiencing performance issues. Requires page reload.
            </div>
        `;
  }
  ...
  private bindEvents(): void {
    ...
    this.$("#toggle-3d-mode")?.addEventListener("click", () => {
      ...
      new ConfirmationModal(
        newValue
          ? "ENABLE 3D MODE? PAGE WILL RELOAD."
          : "DISABLE 3D MODE? PAGE WILL RELOAD.",
        () => {
    ...
    this.$("#toggle-post-processing")?.addEventListener("click", () => {
      const webglService = WebGLService.getInstance();
      const newValue = !webglService.isPostProcessingEnabled();

      new ConfirmationModal(
        newValue
          ? "ENABLE POST-PROCESSING? PAGE WILL RELOAD."
          : "DISABLE POST-PROCESSING? PAGE WILL RELOAD.",
        () => {
  ...
  private async loadProfile(): Promise<void> {
    try {
      ...
      this.profile = await ProfileService.getInstance().getUserProfile(
        user.userId
      );
  ...
  private async saveProfile(): Promise<void> {
    ...
    const displayName = (this.$("#input-displayname") as HTMLInputElement)
      .value;
    ...
    const customAvatar =
      avatarUrl !== this.profile.avatarUrl ? 1 : this.profile.customAvatar;
