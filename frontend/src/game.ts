...
export class GameManager {
  ...
  */
  ...
  private setupEventListeners(): void {
    document.addEventListener("keydown", (e: KeyboardEvent) => {
      ...
      console.log(
        ...
        (activeElement as HTMLElement)?.id
      );
      ...
      if (!isInputFocused && this.isPlaying) {
        ...
        if (
          ...
          e.preventDefault();
          console.log(
            "🎯 [KEYDOWN] Prevented default for key:",
            key,
            "original:",
            e.key
          );
        ...
        console.log(
          "🎯 [KEYDOWN] Key ignored - isInputFocused:",
          isInputFocused,
          "isPlaying:",
          this.isPlaying
        );
      ...
      if (gameCanvas) {
        gameCanvas.addEventListener("click", () => {
          if (this.isPlaying) {
            gameCanvas.focus();
            console.log(
              "🎯 [CANVAS-CLICK] Canvas clicked and focused, activeElement:",
              (document.activeElement as HTMLElement)?.id
            );

            // Test key capture
            console.log(
              "🎯 [CANVAS-CLICK] Testing immediate key capture after focus..."
            );
            const testKeys = () => {
              console.log(
                "🎯 [CANVAS-CLICK] Current keys state after focus:",
                this.keys
              );
            };
        ...
        gameCanvas.addEventListener("keydown", (e: KeyboardEvent) => {
          console.log(
            "🎯 [CANVAS-KEYDOWN] Canvas received keydown directly:",
            e.key
          );
        });
      }
    ...
    const findMatchBtn = document.getElementById(
      "find-match-btn"
  ...
  private startGame(gameData: any): void {
    ...
    console.log(
      "🎮 [START] Game initialization complete, waiting for game state updates"
    );
  }
  ...
  private drawBallWithTrail(): void {
    ...
    const dynamicRadius =
      this.gameState.ballRadius * (1 + normalizedSpeed * 0.2);
    ...
    for (let i = 1; i <= trailLength; i++) {
      ...
      const trailRadius =
        this.gameState.ballRadius * (1 - (i / trailLength) * 0.7);
  ...
  private getBallColors(normalizedSpeed: number): {
    main: string;
    highlight: string;
    trail: string;
  } {
  ...
  private drawPlayerInfo(): void {
    ...
    this.ctx.font = "bold 48px VCR OSD Mono, monospace";
    this.ctx.fillStyle = "#77e6ff";
    this.ctx.shadowColor = "#77e6ff";
    this.ctx.shadowBlur = 10;
        ...
        if (this.websocket) {
          ...
          setTimeout(() => {
            if (this.websocket) {
              ...
              console.log(
                "🎮 [SETTINGS] Sending game settings to backend:",
                gameSettings
              );
