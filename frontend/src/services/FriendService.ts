...
export class FriendService {
    ...
    public async addFriend(userId: number, friendId: number): Promise<boolean> {
        try {
            await Api.post('/api/game/friends/add', { userId, friendId });
            return true;
        } catch (e) {
    ...
    public async removeFriend(userId: number, friendId: number): Promise<boolean> {
        try {
            await Api.post('/api/game/friends/remove', { userId, friendId });
            return true;
        } catch (e) {
    ...
    public async getFriends(userId: number): Promise<Friend[]> {
        try {
            const res = await Api.get(`/api/game/friends/${userId}`);
            return res.data || [];
        } catch (e) {
