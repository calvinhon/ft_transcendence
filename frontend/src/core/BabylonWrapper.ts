...
export class BabylonWrapper {
  ...
  private constructor() {
    ...
    const postProcessingEnabled = webglService.isPostProcessingEnabled();
  ...
  private setupScene(): void {
    ...
    const leftTrigger = CreateBox(
      "leftInteractionTrigger",
    ...
    // Right Trigger
    const rightTrigger = CreateBox(
      "rightInteractionTrigger",
      { width: 0.4, height: 0.6, depth: 0.05 },
      this.scene
    );
  ...
  private setupInteractions(): void {
    ...
    window.addEventListener("wheel", (e) => {
      ...
      const targetRadius = Math.max(
        this.ZOOM_MIN,
        Math.min(this.ZOOM_MAX, this.camera.radius + delta * sensitivity)
      );
      ...
      const currentState = this.isLoreView
    ...
    window.addEventListener("mousemove", (e) => {
      ...
      const currentRadius = Math.max(
        minRadius,
        Math.min(maxRadius, this.camera.radius)
      );
      ...
      if (pointerInfo.type === PointerEventTypes.POINTERDOWN) {
        ...
        if (pickInfo?.hit && pickInfo.pickedMesh) {
          ...
          if (meshName === "leftInteractionTrigger" && !this.isLoreView) {
            ...
            meshName === "rightInteractionTrigger" &&
            this.isLoreView
          ) {
            this.panToMonitor();
          }
  ...
  public async panToLore(): Promise<void> {
    ...
    this.animateCameraTo(
      ...
      targetPos,
      targetFov,
      this.loreCameraState.alpha,
      this.loreCameraState.beta,
      1500
    );
  }
  ...
  private async loadModel(): Promise<void> {
    try {
      await AppendSceneAsync(
        "/assets/models/low_poly_90s_office_cubicle.glb",
        this.scene
      );

      // Find the glowing screen mesh
      const screenMesh = this.scene.meshes.find((m) =>
        m.name.toLowerCase().includes("monitor_mesh")
      );
      this.scene.meshes.find((m) =>
        m.name.toLowerCase().includes("glowing screen")
      )!.isVisible = false;
      if (!screenMesh) {
  ...
  private createHtmlMesh(parentMesh: AbstractMesh | null): void {
    ...
    if (parentMesh) {
      ...
      this.screenLight = new PointLight(
        "screenLight",
        new Vector3(0, 0, -4),
        this.scene
      );
  ...
  public animateCameraTo(
    ...
    this.scene.beginDirectAnimation(
      this.camera,
      [radiusAnim, alphaAnim, betaAnim, targetAnim, fovAnim],
      0,
      totalFrames,
      false,
      1,
