...
export class LocalPlayerAuthManager {
  ...
  public async loginPlayer(email: string, password: string): Promise<any> {
    ...
    try {
      ...
      logger.error('local-player-auth-manager', 'Exception during login', err);
  ...
  public async registerPlayer(username: string, email: string, password: string): Promise<any> {
    ...
    try {
      ...
      logger.error('local-player-auth-manager', 'Exception during registration', err);
  ...
  public async requestPasswordReset(email: string): Promise<boolean> {
    ...
    if (!authManager) {
      logger.error('local-player-auth-manager', 'AuthManager not available');
      return false;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        logger.error('local-player-auth-manager', 'Password reset failed', result.error);
        return false;
      }
    } catch (error) {
      logger.error('local-player-auth-manager', 'Exception during forgot password', error);
      return false;
    }
  }
}