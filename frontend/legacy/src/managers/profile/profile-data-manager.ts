...
export class ProfileDataManager {
  ...
  constructor() {
    logger.info('profile-data-manager', '🏗️ ProfileDataManager initialized');
  }
  ...
  async loadUserProfile(userId: number): Promise<UserProfile | null> {
    try {
      logger.info('profile-data-manager', `Loading user profile for userId: ${userId}`);
      ...
      if (response.ok) {
        ...
        logger.info('profile-data-manager', 'Profile data loaded successfully');
        return userInfo;
      } else {
        logger.warn('profile-data-manager', `Failed to load user profile: ${response.status}`);
        return null;
      }
    } catch (error) {
      logger.error('profile-data-manager', 'Failed to load user profile:', error);
      return null;
    }
  }
  ...
  async loadGameStats(userId: number): Promise<GameStats> {
    try {
      logger.info('profile-data-manager', `Loading game stats for userId: ${userId}`);
      ...
      if (response.ok) {
        ...
        logger.info('profile-data-manager', 'Game stats loaded successfully');
        return stats;
      } else {
        logger.warn('profile-data-manager', `Game stats API returned error: ${response.status}`);
        return this.getDefaultStats();
      }
    } catch (error) {
      logger.error('profile-data-manager', 'Failed to load game stats:', error);
      return this.getDefaultStats();
    }
  }
  ...
  async loadRecentGames(userId: number, limit: number = 20): Promise<RecentGame[]> {
    try {
      logger.info('profile-data-manager', `Loading recent games for userId: ${userId}, limit: ${limit}`);
      ...
      if (response.ok) {
        ...
        if (apiResponse && typeof apiResponse === 'object' && apiResponse.data) {
          ...
          logger.warn('profile-data-manager', 'Game history API returned unexpected format:', apiResponse);
          return [];
        }
        
        // Ensure apiGames is an array
        if (!Array.isArray(apiGames)) {
          logger.warn('profile-data-manager', 'Game history API data is not an array:', apiGames);
          return [];
        }
        ...
        logger.info('profile-data-manager', `Loaded ${games.length} recent games`);
        return games;
      } else {
        logger.warn('profile-data-manager', `Game history API returned error: ${response.status}`);
        return [];
      }
    } catch (error) {
      logger.error('profile-data-manager', 'Failed to load recent games:', error);
      return [];
    }
  }
  ...
  async loadTournamentCount(userId: number): Promise<number> {
    try {
      logger.info('profile-data-manager', `Loading tournament count for userId: ${userId}`);
      ...
      if (response.ok) {
        ...
        logger.info('profile-data-manager', `Tournament count: ${count}`);
        return count;
      } else {
        logger.warn('profile-data-manager', `Tournament count API returned error: ${response.status}`);
        return 0;
      }
    } catch (error) {
      logger.error('profile-data-manager', 'Failed to load tournament count:', error);
      return 0;
    }
  }
  ...
  async loadTournamentRankings(userId: number): Promise<TournamentRanking[]> {
    try {
      logger.info('profile-data-manager', `Loading tournament rankings for userId: ${userId}`);
      ...
      if (response.ok) {
        ...
        if (apiResponse && typeof apiResponse === 'object' && apiResponse.data) {
          ...
          logger.warn('profile-data-manager', 'Tournament rankings API returned unexpected format:', apiResponse);
          return [];
        }
        
        logger.info('profile-data-manager', `Loaded ${rankings.length} tournament rankings`);
        return rankings;
      } else {
        logger.warn('profile-data-manager', `Tournament rankings API returned error: ${response.status}`);
        return [];
      }
    } catch (error) {
      logger.error('profile-data-manager', 'Failed to load tournament rankings:', error);
      return [];
    }
  }
  ...
  private async fetchOpponentName(opponentId: number): Promise<string> {
    try {
      ...
      logger.error('profile-data-manager', 'Error fetching opponent profile:', error);
