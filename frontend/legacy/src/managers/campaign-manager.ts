  ...
  private async syncLevelToBackend(): Promise<void> {
    try {
      ...
      if (!response.ok) {
        ...
        logger.error('campaign-manager', `Failed to sync level to server: ${response.status} ${errorText}`);
      } else {
        const responseData = await response.json();
        console.log('✅ [CAMPAIGN] Level synced to database:', responseData);
      }
    } catch (error) {
      logger.error('campaign-manager', 'Exception syncing level to server', error);
    }
  }
