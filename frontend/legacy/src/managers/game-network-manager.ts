...
export class GameNetworkManager {
  ...
  public async connectToArcadeGame(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.websocket.onerror = (error) => {
        logger.error('game-network-manager', 'WebSocket error during arcade connection', error);
        reject(error);
      };
    });
  }
  ...
  public async connectToBotGame(user: any): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.websocket.onerror = (error) => {
        logger.error('game-network-manager', 'Bot game server connection error', error);
        reject(error);
      };
    });
  }
  ...
  public async connectToCampaignGame(user: any): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.websocket.onerror = (error) => {
        logger.error('game-network-manager', 'Campaign game server connection error', error);
        reject(error);
      };
    });
  }
  ...
  private handleGameMessage(event: MessageEvent): void {
    try {
      ...
      logger.error('game-network-manager', 'Error parsing game message', error);
    }
  }
  ...
  public sendArcadeMatchRequest(
    ...
    if (!this.websocket || this.websocket.readyState !== WebSocket.OPEN) {
      logger.error('game-network-manager', 'Cannot send arcade request: WebSocket not connected');
      return;
    }
