...
export class ProfileUIManager {
  constructor() {
    logger.info('profile-ui-manager', '🏗️ ProfileUIManager initialized');
  }
  ...
  displayUserProfile(userInfo: UserProfile): void {
    logger.info('profile-ui-manager', 'Displaying user profile information');
    const user = authService?.getCurrentUser();
  ...
  displayGameStats(stats: GameStats): void {
    logger.info('profile-ui-manager', 'Displaying game statistics');
  ...
  displayRecentGames(games: RecentGame[]): void {
    logger.info('profile-ui-manager', `Displaying ${games.length} recent games`);

    const container = document.getElementById('profile-recent-activity');
    if (!container) {
      logger.warn('profile-ui-manager', 'Recent activity container not found!');
      return;
    }
  ...
  displayTournamentRankings(rankings: TournamentRanking[]): void {
    logger.info('profile-ui-manager', `Displaying ${rankings.length} tournament rankings`);

    const container = document.getElementById('profile-tournament-rankings');
    if (!container) {
      logger.warn('profile-ui-manager', 'Tournament rankings container not found!');
      return;
    }
