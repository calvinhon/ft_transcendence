...
export class SettingsManager {
  ...
  private loadSettings(): GameSettings {
    try {
      ...
      if (saved) {
        ...
        logger.info('settings-manager', 'Loaded settings from localStorage', parsed);
        return { ...this.getDefaultSettings(), ...parsed };
      }
    } catch (error) {
      logger.warn('settings-manager', 'Failed to load settings from localStorage', error);
    }
  ...
  private saveSettings(): void {
    try {
      ...
      logger.debug('settings-manager', 'Settings saved to localStorage');
    } catch (error) {
      logger.error('settings-manager', 'Failed to save settings to localStorage', error);
    }
  }
  ...
  updateSetting<K extends keyof GameSettings>(key: K, value: GameSettings[K]): void {
    ...
    logger.info('settings-manager', `Setting updated: ${key}`, { from: oldValue, to: value });
  ...
  updateSettings(updates: Partial<GameSettings>): void {
    ...
    logger.info('settings-manager', 'Multiple settings updated', { updates, oldSettings });
  ...
  resetToDefaults(): void {
    ...
    logger.info('settings-manager', 'Settings reset to defaults', { oldSettings });
  ...
  setupUIListeners(): void {
    ...
    logger.info('settings-manager', 'UI listeners setup complete');
  }
  ...
  private handleConfigOption(setting: string, value: string, button: HTMLElement): void {
    ...
    if (settingKey in this.settings) {
      ...
      logger.info('settings-manager', `Setting updated via UI: ${settingKey} = ${value}`);
  ...
  updateUI(): void {
    ...
    logger.debug('settings-manager', 'UI updated to reflect current settings');
  }
  ...
  importSettings(jsonString: string): boolean {
    try {
      ...
      logger.info('settings-manager', 'Settings imported successfully', validated);
      return true;
    } catch (error) {
      logger.error('settings-manager', 'Failed to import settings', error);
      return false;
    }
  }
}
