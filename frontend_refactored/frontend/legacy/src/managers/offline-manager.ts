...
export class OfflineManager {
  ...
  private initializeOfflineSupport(): void {
    ...
    logger.info('offline-manager', `Offline support initialized. Online: ${this.isOnline}`);
  }
  ...
  private async registerServiceWorker(): Promise<void> {
    if ('serviceWorker' in navigator) {
      try {
        ...
        logger.info('offline-manager', 'Service worker registered successfully');
        ...
        logger.warn('offline-manager', 'Service worker registration failed', error);
      }
    }
  }
  ...
  private handleOnline(): void {
    ...
    logger.info('offline-manager', 'Device came online');
    ...
    if (wasOffline) {
      ...
      this.observers.forEach(observer => {
        try {
          ...
          logger.error('offline-manager', 'Error in online observer', error);
        }
      });
  ...
  private handleOffline(): void {
    ...
    logger.info('offline-manager', 'Device went offline');
    ...
    this.observers.forEach(observer => {
      try {
        ...
        logger.error('offline-manager', 'Error in offline observer', error);
      }
    });
  }
  ...
  public queueForSync(action: string, data: any): string {
    ...
    logger.debug('offline-manager', `Queued action for sync: ${action}`, syncItem);
  ...
  public removeFromSyncQueue(syncId: string): void {
    ...
    if (index > -1) {
      ...
      this.saveSyncQueue();
      logger.debug('offline-manager', `Removed sync item: ${syncId}`);
    }
  }
  ...
  private async syncPendingData(): Promise<void> {
    ...
    logger.info('offline-manager', `Starting sync of ${this.syncQueue.length} pending items`);
    ...
    for (const item of itemsToSync) {
      try {
        ...
        logger.debug('offline-manager', `Successfully synced item: ${item.action}`);
      } catch (error) {
        logger.warn('offline-manager', `Failed to sync item ${item.action}:`, error);
        ...
        if (error && typeof error === 'object' && 'status' in error && (error as any).status >= 400 && (error as any).status < 500) {
          this.removeFromSyncQueue(item.id);
          logger.warn('offline-manager', `Removed permanently failed sync item: ${item.action}`);
        }
    ...
    logger.info('offline-manager', 'Sync completed');
  }
  ...
  private async syncItem(item: any): Promise<void> {
    switch (item.action) {
      ...
      case 'tournament_action':
        ...
        break;
      default:
        logger.warn('offline-manager', `Unknown sync action: ${item.action}`);
    }
  }
  ...
  private saveSyncQueue(): void {
    try {
      ...
      logger.error('offline-manager', 'Failed to save sync queue', error);
    }
  }
  ...
  private loadSyncQueue(): void {
    try {
      ...
      if (stored) {
        ...
        logger.info('offline-manager', `Loaded ${this.syncQueue.length} pending sync items`);
      }
    } catch (error) {
      logger.error('offline-manager', 'Failed to load sync queue', error);
      this.syncQueue = [];
    }
  }
  ...
  public cacheData(key: string, data: any): void {
    try {
      ...
      logger.debug('offline-manager', `Cached data: ${key}`);
    } catch (error) {
      logger.error('offline-manager', `Failed to cache data: ${key}`, error);
    }
  }
  ...
  public getCachedData(key: string): any | null {
    try {
      ...
      logger.error('offline-manager', `Failed to get cached data: ${key}`, error);
      return null;
    }
  }
  ...
  public clearExpiredCache(): void {
    try {
      ...
      if (cleared > 0) {
        logger.info('offline-manager', `Cleared ${cleared} expired cache items`);
      }
    } catch (error) {
      logger.error('offline-manager', 'Failed to clear expired cache', error);
    }
  }
  ...
  private notifyUpdateAvailable(): void {
    logger.info('offline-manager', 'New app version available');
  ...
  public async registerBackgroundSync(): Promise<void> {
    if (!this.isBackgroundSyncSupported()) {
      logger.warn('offline-manager', 'Background sync not supported');
      return;
    }
    ...
    try {
      ...
      logger.info('offline-manager', 'Background sync registered');
    } catch (error) {
      logger.error('offline-manager', 'Failed to register background sync', error);
    }
  }
}
