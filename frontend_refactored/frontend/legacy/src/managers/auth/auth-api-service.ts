...
export class AuthApiService {
  ...
  async register(username: string, email: string, password: string): Promise<AuthResult> {
    ...
    try {
      ...
      logger.error('auth-api-service', 'Registration catch block error', error);
      logger.error('auth-api-service', `Error type: ${typeof error}`);
      logger.error('auth-api-service', `Error message: ${error instanceof Error ? error.message : error}`);
  ...
  async forgotPassword(email: string): Promise<AuthResult> {
    try {
      ...
      logger.error('auth-api-service', 'Forgot password error', error);
      return { success: false, error: 'Network error' };
    }
  }
}