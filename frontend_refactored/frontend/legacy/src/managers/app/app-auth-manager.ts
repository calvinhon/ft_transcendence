...
export class AppAuthManager {
  ...
  constructor(app: any) {
    this.app = app;
    logger.info('app-auth-manager', '🏗️ AppAuthManager initialized');
  }
  ...
  async handleLogin(username: string, password: string): Promise<void> {
    logger.info('app-auth-manager', `🔐 Handling login for user: ${username}`);

    try {
      const result = await handleHostLogin(username, password, authService);

      if (result.success) {
        logger.info('app-auth-manager', '✅ Login successful');
        ...
        if (currentUser && playerService && typeof playerService.setHostUser === 'function') {
          ...
          logger.info('app-auth-manager', '✅ Host user set in PlayerManager');
        }
        ...
        logger.warn('app-auth-manager', `❌ Login failed: ${result.error}`);
        showToast(result.error || 'Login failed', 'error');
      }
    } catch (error) {
      logger.error('app-auth-manager', '💥 Login error:', error);
  ...
  async handleRegister(username: string, email: string, password: string): Promise<void> {
    logger.info('app-auth-manager', `📝 Handling registration for user: ${username}`);

    try {
      const result = await handleHostRegister(username, email, password, authService);

      if (result.success) {
        logger.info('app-auth-manager', '✅ Registration successful');
        showToast('Registration successful! Please login.', 'success');
        await this.app.router.navigateToScreen('login');
      } else {
        logger.warn('app-auth-manager', `❌ Registration failed: ${result.error}`);
        showToast(result.error || 'Registration failed', 'error');
      }
    } catch (error) {
      logger.error('app-auth-manager', '💥 Registration error:', error);
  ...
  async handleForgotPassword(email: string): Promise<void> {
    logger.info('app-auth-manager', `🔑 Handling forgot password for: ${email}`);
    ...
    try {
      ...
      try {
        ...
        logger.error('app-auth-manager', 'authService.forgotPassword failed', err);
        throw err;
      }

      if (result.success) {
        logger.info('app-auth-manager', '✅ Forgot password request successful');
        showToast('Password reset email sent!', 'success');
        await this.app.router.navigateToScreen('login');
      } else {
        logger.warn('app-auth-manager', `❌ Forgot password failed: ${result.error}`);
        showToast(result.error || 'Failed to send reset email', 'error');
      }
    } catch (error) {
      logger.error('app-auth-manager', '💥 Forgot password error:', error);
  ...
  async handleLogout(): Promise<void> {
    logger.info('app-auth-manager', '🚪 Handling logout');
    ...
    try {
      try {
        ...
        logger.error('app-auth-manager', 'authService.logout failed', err);
      }
      ...
      if (playerService && typeof playerService.clearHostUser === 'function') {
        ...
        logger.info('app-auth-manager', '✅ Host user cleared in PlayerManager');
      }

      logger.info('app-auth-manager', '✅ Logout successful');
      showToast('Logged out successfully', 'info');
      // Navigate back to login screen
      try {
        await this.app.router.navigateToScreen('login');
      } catch (navErr) {
        logger.warn('app-auth-manager', 'Router navigation failed during logout', navErr);
      }
      ...
      try {
        ...
        logger.warn('app-auth-manager', 'App UI update failed; attempting manager fallbacks', uiErr);
      }
      ...
      try {
        try {
          ...
          logger.warn('app-auth-manager', 'playerService.clearAllPlayers failed', e);
        }
        ...
        logger.error('app-auth-manager', 'Fallback UI manager reset failed', fallbackErr);
      }
    } catch (error) {
      logger.error('app-auth-manager', '💥 Logout error:', error);
