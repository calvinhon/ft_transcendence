...
import { LocalPlayerManager } from '../local-player/local-player-manager';

export class AppGameModeManager {
  constructor() {
    logger.info('app-game-mode-manager', '🏗️ AppGameModeManager initialized');
  }
  ...
  handleGameModeChange(tab: HTMLElement): void {
    ...
    logger.info('app-game-mode-manager', `Game mode changed to: ${mode}`);
