...
export class PlayerManager {
  ...
  private setupEventListeners(): void {
    ...
    logger.info('player-manager', 'Event listeners setup');
  }
  ...
  private handlePlayerClick(e: Event): void {
    ...
    if (target.closest('.add-player-btn')) {
      logger.debug('player-manager', 'Add player button clicked, allowing propagation');
      return;
    }
    ...
    if (!isInsideParty) {
      logger.debug('player-manager', 'Click outside party container, ignoring');
      return;
    }
  ...
  public togglePlayerSelection(card: HTMLElement): void {
    ...
    if (isNowActive) {
      ...
      logger.debug('player-manager', `Player ${playerId} selected`);
    } else {
      this.selectedPlayerIds.delete(playerId);
      logger.debug('player-manager', `Player ${playerId} deselected`);
    }
  ...
  private updatePlayerSelectionUI(): void {
    ...
    logger.debug('player-manager', 'Player selection UI update requested');
  }
  ...
  setHostUser(user: User): void {
    this.hostUser = user;
    logger.info('player-manager', 'Host user set', user);
  }
  ...
  clearHostUser(): void {
    this.hostUser = null;
    logger.info('player-manager', 'Host user cleared');
  }
  ...
  addLocalPlayer(player: LocalPlayer): void {
    ...
    logger.info('player-manager', 'Local player added', player);
  }
  ...
  removeLocalPlayer(playerId: string): void {
    ...
    if (index >= 0) {
      ...
      logger.info('player-manager', 'Local player removed', removed);
    }
  }
  ...
  setSelectedPlayers(playerIds: string[]): void {
    ...
    this.updateUI();
    logger.debug('player-manager', 'Selected players set', playerIds);
  }
  ...
  clearSelections(): void {
    ...
    this.updateUI();
    logger.debug('player-manager', 'All selections cleared');
  }
  ...
  updateUI(): void {
    ...
    logger.debug('player-manager', 'UI update requested');
  }
  ...
  private loadFromLocalStorage(): void {
    try {
      ...
      if (saved) {
        ...
        logger.info('player-manager', 'Loaded players from localStorage', this.localPlayers.length);
      }
    } catch (error) {
      logger.warn('player-manager', 'Failed to load players from localStorage', error);
      this.localPlayers = [];
    }
  }
  ...
  private saveToLocalStorage(): void {
    try {
      ...
      logger.debug('player-manager', 'Players saved to localStorage');
    } catch (error) {
      logger.error('player-manager', 'Failed to save players to localStorage', error);
    }
  }
  ...
  clearAllPlayers(): void {
    ...
    this.saveToLocalStorage();
    this.updateUI();
    logger.info('player-manager', 'All players cleared');
  }
