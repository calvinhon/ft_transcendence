...
export class TournamentGameManager {
  ...
  public handleTournamentGameEnd(
    ...
    if (!user) {
      logger.error('tournament-game-manager', 'No user logged in');
      return;
    }
    ...
    if (actualWinnerId !== originalPlayer1Id && actualWinnerId !== originalPlayer2Id) {
      logger.error('tournament-game-manager', 'Winner ID does not match any original player', {
        winnerId: actualWinnerId,
        originalPlayer1Id,
        originalPlayer2Id
      });
    ...
    this.recordMatchResult(
      ...
      logger.error('tournament-game-manager', 'Failed to record match result', error);
