...
export class TournamentNetworkManager {
  ...
  public async loadTournaments(): Promise<Tournament[]> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        logger.error('tournament-network-manager', `Failed to load tournaments: ${response.status} ${response.statusText}`);
        const errorBody = await response.text();
        logger.error('tournament-network-manager', `Error body: ${errorBody}`);
        throw new Error(`Failed to load tournaments: ${response.status}`);
      }
    } catch (error) {
      logger.error('tournament-network-manager', 'Error loading tournaments', error);
      throw error;
    }
  }
  ...
  public async createTournament(tournamentData: {
    ...
    try {
      ...
      logger.error('tournament-network-manager', 'Create tournament error', error);
      throw error;
    }
  }
  ...
  public async joinTournament(tournamentId: number, userId: number): Promise<any> {
    try {
      ...
      if (response.ok) {
        ...
        logger.error('tournament-network-manager', `Join failed: ${errorMessage}`);
        throw new Error(errorMessage);
      }
    } catch (error) {
      logger.error('tournament-network-manager', 'Join tournament network error', error);
      throw error;
    }
  }
  ...
  public async addParticipant(tournamentId: number, userId: number): Promise<any> {
    try {
      ...
      if (!joinResponse.ok) {
        ...
        logger.error('tournament-network-manager', `Failed to add participant: ${userId}`, joinResult);
      ...
      logger.error('tournament-network-manager', 'Add participant error', error);
      throw error;
    }
  }
  ...
  public async startTournament(tournamentId: number): Promise<any> {
    try {
      ...
      logger.error('tournament-network-manager', 'Start tournament error', error);
      throw error;
    }
  }
  ...
  public async viewTournament(tournamentId: number): Promise<any> {
    try {
      ...
      const participantsWithNames = await Promise.all(
        details.participants.map(async (p: any) => {
          try {
            ...
            logger.error('tournament-network-manager', `Failed to fetch username for user ${p.user_id}`, err);
          }
      ...
      logger.error('tournament-network-manager', 'View tournament error', error);
      throw error;
    }
  }
  ...
  public async recordMatchResult(tournamentId: number, matchId: number, winnerId: number, player1Score: number, player2Score: number): Promise<any> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.error('tournament-network-manager', 'Failed to record match result', errorData);
      ...
      logger.error('tournament-network-manager', 'Record match result error', error);
      throw error;
    }
  }
  ...
  public async recordOnBlockchain(tournamentId: number, winnerId: number): Promise<any> {
    try {
      ...
      logger.error('tournament-network-manager', 'Blockchain recording error', error);
      throw error;
    }
  }
}