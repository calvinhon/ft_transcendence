...
export class PongGame {
        ...
        for (const paddle of this.paddles.team1) {
          if (this.ball.y >= paddle.y && this.ball.y <= paddle.y + 100) {
            ...
            if (this.accelerateOnHit) {
              ...
              logger.game(this.gameId, `Ball accelerated! ${oldSpeed.toFixed(1)} → ${newSpeed.toFixed(1)}`);
            }
            leftHit = true;
            break;
          }
  ...
  resetBall(direction?: 'left' | 'right'): void {
    ...
    logger.game(this.gameId, `Ball reset${direction ? ' toward ' + direction : ''}. Current scores: Player1=${this.scores.player1}, Player2=${this.scores.player2}`);

    // Brief delay without countdown overlay - just freeze ball for 1 second
    setTimeout(() => {
      this.ballFrozen = false;
      logger.game(this.gameId, `Ball unfrozen!`);
      this.broadcastGameState();
    }, 1000); // 1 second delay
  ...
  movePaddle(playerId: number, direction: 'up' | 'down', paddleIndex?: number): void {
    logger.gameDebug(this.gameId, 'Called with playerId:', playerId, 'direction:', direction, 'paddleIndex:', paddleIndex);
    logger.gameDebug(this.gameId, 'player1.userId:', this.player1.userId, 'player2.userId:', this.player2.userId);
    logger.gameDebug(this.gameId, 'gameMode:', this.gameSettings.gameMode);
    ...
    if ((this.gameSettings.gameMode === 'arcade' || this.gameSettings.gameMode === 'tournament') && paddleIndex !== undefined) {
      ...
      if (!paddles || !paddles[paddleIndex]) {
        logger.gameDebug(this.gameId, 'Invalid paddle index:', paddleIndex, 'for team:', team);
        return;
      }
      ...
      if (direction === 'up' && paddle.y > 0) {
        ...
        logger.gameDebug(this.gameId, `Team ${team} paddle ${paddleIndex} moved UP from ${oldY} to ${paddle.y}`);
      } else if (direction === 'down' && paddle.y < 500) {
        paddle.y = Math.min(500, paddle.y + moveSpeed);
        logger.gameDebug(this.gameId, `Team ${team} paddle ${paddleIndex} moved DOWN from ${oldY} to ${paddle.y}`);
      } else {
        logger.gameDebug(this.gameId, `Movement blocked for team ${team} paddle ${paddleIndex}`);
      }
      return;
    }
    ...
    logger.gameDebug(this.gameId, 'Determined paddle:', paddle);

    if (!this.paddles[paddle]) {
      logger.gameDebug(this.gameId, 'Invalid player for paddle movement:', playerId);
      return;
    }

    const oldY = this.paddles[paddle].y;
    logger.gameDebug(this.gameId, 'Current paddle Y:', oldY, 'Max Y (500):', 500);
    ...
    if (direction === 'up' && this.paddles[paddle].y > 0) {
      ...
      logger.gameDebug(this.gameId, 'Moved UP from', oldY, 'to', this.paddles[paddle].y);
    } else if (direction === 'down' && this.paddles[paddle].y < 500) {
      this.paddles[paddle].y = Math.min(500, this.paddles[paddle].y + moveSpeed);
      logger.gameDebug(this.gameId, 'Moved DOWN from', oldY, 'to', this.paddles[paddle].y);
    } else {
      logger.gameDebug(this.gameId, 'Movement blocked - direction:', direction, 'currentY:', this.paddles[paddle].y, 'bounds: [0, 500]');
      if (direction === 'up' && this.paddles[paddle].y <= 0) {
        logger.gameDebug(this.gameId, 'Cannot move up - already at top boundary');
      } else if (direction === 'down' && this.paddles[paddle].y >= 500) {
        logger.gameDebug(this.gameId, 'Cannot move down - already at bottom boundary');
      }
    }
    ...
    if (oldY !== this.paddles[paddle].y) {
      ...
      logger.gameDebug(this.gameId, 'Broadcasting game state update - paddle moved', (this.paddles[paddle].y - oldY), 'pixels');
      this.broadcastGameState();
    } else {
      logger.gameDebug(this.gameId, 'No movement occurred, not broadcasting');
    }
  }
  ...
  broadcastGameState(): void {
    ...
    logger.gameDebug(this.gameId, 'Broadcasting game state:', JSON.stringify(gameState));
  ...
  endGame(): void {
    ...
    logger.game(this.gameId, `Winner: ${winnerId === this.player1.userId ? this.player1.username : this.player2.username}`);
