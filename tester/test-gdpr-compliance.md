...
# Delete user account
curl -X DELETE "https://user:3000/gdpr/delete/$USER_ID" \
  -H "Authorization: Bearer $TOKEN" | jq .
...
# Update profile
curl -X PUT "https://user:3000/profile" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
curl -s -X GET "https://user:3000/gdpr/status/1" \
...
time curl -X GET "https://user:3000/gdpr/export/1" \
  -H "Authorization: Bearer $TOKEN" > /dev/null
...
for i in {1..10}; do
  time curl -s -X GET "https://user:3000/gdpr/export/1" \
    -H "Authorization: Bearer $TOKEN" > /dev/null
done
...
curl -X GET "https://user:3000/profile/sharing-preferences" \
...
TOKEN=$(curl -s -X POST "https://auth:3000/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"username":"gdpr_test","password":"GdprTest123!"}' \
  | jq -r '.token')

# 3. Export data (Right to Access)
curl -X GET "https://user:3000/gdpr/export/[USER_ID]" \
  -H "Authorization: Bearer $TOKEN" | jq '.user.email'
# Expected: gdpr@example.com

# 4. Check GDPR status
curl -X GET "https://user:3000/gdpr/status/[USER_ID]" \
  -H "Authorization: Bearer $TOKEN" | jq '.rights | keys'
...
# 6. Anonymize account
curl -X POST "https://user:3000/gdpr/anonymize/[USER_ID]" \
  -H "Authorization: Bearer $TOKEN" | jq '.'
...
```bash
# Get GDPR status
curl https://user:3000/gdpr/status/1 -H "Authorization: Bearer $TOKEN" | jq

# Export user data
curl https://user:3000/gdpr/export/1 -H "Authorization: Bearer $TOKEN" | jq '.user'
