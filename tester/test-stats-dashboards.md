...
```bash
# Backend API test:
curl -X GET "https://user:3000/stats/user/1" \
...
```bash
# Get initial stats
INITIAL=$(curl -s "https://user:3000/stats/user/1" \
...
# Get updated stats
UPDATED=$(curl -s "https://user:3000/stats/user/1" \
  -H "Authorization: Bearer $TOKEN" | jq '.stats')
...
```bash
# Get detailed stats
curl -X GET "https://user:3000/stats/user/1/games" \
...
# API test:
curl -X GET "https://user:3000/leaderboard?limit=10" \
...
```bash
# Get player 1 stats
PLAYER1=$(curl -s "https://user:3000/stats/user/1" \
  -H "Authorization: Bearer $TOKEN" | jq '.stats')

# Get player 2 stats
PLAYER2=$(curl -s "https://user:3000/stats/user/2" \
  -H "Authorization: Bearer $TOKEN" | jq '.stats')
...
```bash
# Get page 1
curl -X GET "https://user:3000/leaderboard?page=1&limit=10" \
  -H "Authorization: Bearer $TOKEN" | jq '.pagination'

# Get page 2
curl -X GET "https://user:3000/leaderboard?page=2&limit=10" \
  -H "Authorization: Bearer $TOKEN" | jq '.pagination'
...
```bash
# Profile endpoint
PROFILE=$(curl -s "https://user:3000/profile" \
  -H "Authorization: Bearer $TOKEN" | jq '.stats')

# Leaderboard endpoint
LEADERBOARD=$(curl -s "https://user:3000/leaderboard" \
  -H "Authorization: Bearer $TOKEN" | jq '.leaderboard[0].stats')

# Game history endpoint
HISTORY=$(curl -s "https://user:3000/games" \
...
time curl -s "https://user:3000/leaderboard?limit=100" \
...
```bash
# API tests
curl -X GET https://user:3000/profile -H "Authorization: Bearer $TOKEN" | jq
curl -X GET https://user:3000/leaderboard -H "Authorization: Bearer $TOKEN" | jq
curl -X GET https://user:3000/stats/user/1 -H "Authorization: Bearer $TOKEN" | jq
