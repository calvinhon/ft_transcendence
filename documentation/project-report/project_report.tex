...
\begin{table}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/risk_matrix.jpeg}
\caption{Risk Matrix}
...
The network perimeter enforces HTTPS (TLS 1.2+), NGINX reverse proxying, and ModSecurity WAF inspection (OWASP CRS) for ingress traffic control and attack filtering.

\subsection{Layer 2: Input Validation}

All external payloads are validated before business logic execution using:
\begin{itemize}
  \item \textbf{JSON Schema Validation:} Request body and parameter shape enforcement
  \item \textbf{Type Checking:} Strict TypeScript typing and runtime checks
  \item \textbf{Length Constraints:} Bounded input sizes for fields and payloads
\end{itemize}

\subsection{Layer 3: Application Logic}

Application-level protections address core web attack classes:
\begin{itemize}
  \item \textbf{Parameterized SQL:} Prepared statements with \texttt{?} placeholders to prevent SQL injection
  \item \textbf{CSRF Protection:} SameSite cookie policies and origin checks
  \item \textbf{XSS Prevention:} Input sanitization and safe rendering patterns
\end{itemize}

\subsection{Layer 4: Authentication}

Identity assurance is implemented through:
\begin{itemize}
  \item \textbf{Bcrypt (cost 10):} Password hashing with computational work factor
  \item \textbf{OAuth 2.0:} External identity provider integration (Google)
\end{itemize}

\subsection{Layer 5: Authorization}

Resource-level permissions are applied so authenticated users can only access data and operations explicitly permitted by role and ownership rules.

\subsection{Layer 6: Data Protection}

Sensitive data is protected through:
\begin{itemize}
  \item \textbf{Encryption at Rest:} Storage-level and secret-backed data protection controls
  \item \textbf{Vault Secrets:} Centralized secret storage and retrieval via HashiCorp Vault
  \item \textbf{Rotation Policy:} Periodic rotation of keys, tokens, and certificates
\end{itemize}

\subsubsection{Threat Coverage Summary}
This layered model is designed to prevent \textbf{SQLi}, \textbf{XSS}, \textbf{CSRF}, \textbf{DDoS}, and \textbf{data breaches}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/login_and_registration_user_flow.jpeg}
\caption{User Authentication Journey}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{figures/game_flow.jpeg}
\caption{Match Selection to Gameplay Journey (Arcade Mode)}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/tournament_flow.jpeg}
\caption{Tournament Set Up to Gameplay Journey}
...
\subsubsection{Dashboard and Profile Interface}
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{figures/social_flow.jpeg}
\caption{User Profile Flow}
...
\bibitem{hastam_risk_matrix}
Hastam (2019) Risk Assessment Matrix. Available at: https://i0.wp.com/bizepic.com/wp-content/uploads/2019/12/Risk-Assessment-Matrix-795.jpg (Accessed: March 2026).
