...
\node (decision2) [decision, below of=deploy] {Complete?};
\node (evolution) [process, right of=decision2, xshift=3cm] {Evolution};
\node (end) [process, below of=decision2] {Closure};
...
\begin{tikzpicture}[node distance=2cm, auto]
    ...
    \node[decision, below of=homepage] (auth_check) {Authenticated?};
    \node[process, left of=auth_check, xshift=-2cm] (login) {Login/Register};
    \node[process, right of=auth_check, xshift=2cm] (main_menu) {Main Menu};
    \node[process, below of=main_menu] (game_modes) {Game Modes};
    \node[decision, below of=game_modes] (mode_select) {Select Mode};
    ...
    \node[process, below of=mode_select, yshift=-2cm] (tournament) {Tournament};
...
\begin{tikzpicture}[node distance=1.5cm, auto]
    ...
    \node[decision, below of=start] (has_account) {Has Account?};
    \node[process, left of=has_account, xshift=-2cm] (register) {Registration Form};
    \node[data, below of=register] (validate_reg) {Validate Input};
    \node[decision, below of=validate_reg] (reg_valid) {Valid?};
    \node[process, left of=reg_valid, xshift=-1.5cm] (reg_error) {Show Errors};
    \node[process, right of=reg_valid, xshift=1.5cm] (create_account) {Create Account};
    \node[data, below of=create_account] (store_user) {Store User Data};
    \node[process, below of=store_user] (send_welcome) {Send Welcome Email};
    \node[process, right of=has_account, xshift=2cm] (login) {Login Form};
    \node[data, below of=login] (validate_login) {Validate Credentials};
    \node[decision, below of=validate_login] (login_valid) {Valid?};
    \node[process, left of=login_valid, xshift=-1.5cm] (login_error) {Show Error};
    \node[process, right of=login_valid, xshift=1.5cm] (generate_token) {Generate JWT Token};
    ...
    \draw[->] (store_user) -- (send_welcome);
    \draw[->] (send_welcome) -- (authenticated);
...
\begin{tikzpicture}[node distance=1.5cm, auto]
    ...
    \node[decision, below of=connect_ws] (ws_connected) {Connected?};
    \node[process, left of=ws_connected, xshift=-2cm] (retry_ws) {Retry Connection};
    \node[process, right of=ws_connected, xshift=2cm] (load_assets) {Load Game Assets};
    ...
    \node[decision, below of=game_loop] (game_end) {Game Over?};
    \node[process, left of=game_end, xshift=-2cm] (continue_loop) {Continue Loop};
    \node[process, right of=game_end, xshift=2cm] (calculate_score) {Calculate Final Score};
    \node[data, below of=calculate_score] (save_results) {Save Match Results};
    \node[process, below of=save_results] (update_stats) {Update Player Stats};
    \node[decision, below of=update_stats] (tournament) {Tournament Match?};
    \node[process, left of=tournament, xshift=-2cm] (regular_end) {End Regular Game};
    \node[process, right of=tournament, xshift=2cm] (record_blockchain) {Record on Blockchain};
    \node[process, below of=record_blockchain] (update_bracket) {Update Tournament Bracket};
    \node[startend, below of=regular_end] (end) {Game Complete};
...
\begin{tikzpicture}[node distance=2cm, auto]
    ...
    \tikzstyle{database} = [cylinder, minimum width=2cm, minimum height=1cm, text centered, draw=black, fill=green!30, shape border rotate=90]
    \tikzstyle{external} = [rectangle, minimum width=2.5cm, minimum height=0.8cm, text centered, draw=black, fill=orange!30]
    \tikzstyle{flow} = [->, thick]

    % Frontend
    \node[service] (frontend) {Frontend (React/Three.js)};
    
    % Services
    \node[service, below of=frontend, xshift=-4cm] (auth_service) {Auth Service};
    \node[service, right of=auth_service, xshift=4cm] (user_service) {User Service};
    \node[service, below of=auth_service, xshift=4cm] (game_service) {Game Service};
    \node[service, left of=game_service, xshift=-4cm] (tournament_service) {Tournament Service};
    
    % Databases
    \node[database, below of=auth_service] (auth_db) {Auth DB};
    \node[database, below of=user_service] (user_db) {User DB};
    \node[database, below of=game_service] (game_db) {Game DB};
    \node[database, below of=tournament_service] (tournament_db) {Tournament DB};
    
    % External Services
    \node[external, right of=user_service, xshift=3cm] (blockchain) {Blockchain Service};
    \node[external, below of=blockchain] (redis) {Redis Cache};
    \node[external, below of=redis] (vault) {HashiCorp Vault};
    
    % Connections
    \draw[flow] (frontend) -- (auth_service);
    \draw[flow] (frontend) -- (user_service);
    \draw[flow] (frontend) -- (game_service);
    \draw[flow] (frontend) -- (tournament_service);
    
    \draw[flow] (auth_service) -- (auth_db);
    \draw[flow] (user_service) -- (user_db);
    \draw[flow] (game_service) -- (game_db);
    \draw[flow] (tournament_service) -- (tournament_db);
    
    \draw[flow] (tournament_service) -- (blockchain);
    \draw[flow] (user_service) -- (redis);
    \draw[flow] (auth_service) -- (vault);
    
    % Cross-service communication
    \draw[flow, dashed] (auth_service) -- (user_service);
    \draw[flow, dashed] (user_service) -- (game_service);
    \draw[flow, dashed] (game_service) -- (tournament_service);
\end{tikzpicture}
\caption{Data Flow Architecture: Service interactions and data persistence layers}
\label{fig:data_flow}
\end{figure}
