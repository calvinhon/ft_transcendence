...
\begin{itemize}
  ...
  \item FR-15: Tournament results shall be recorded immutably
  \item FR-16: Users shall view tournament standings and schedules
\end{itemize}

\subsubsection{Usability \& User Experience}
\begin{itemize}
  \item FR-17: Interface shall be responsive across desktop and mobile devices
  \item FR-18: 3D mode shall gracefully degrade to 2D rendering
  \item FR-19: User onboarding shall take $<$5 minutes for new users
  \item FR-20: Error messages shall be clear and actionable
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Frontend (Nginx):} Vanilla TypeScript SPA with Babylon.js 3D rendering, served via Nginx with ModSecurity WAF
\end{enumerate}
...
The implementation follows a microservices architecture with five backend services communicating via REST APIs and WebSocket connections, supported by infrastructure containers (Vault, Redis, Nginx, Hardhat). The system achieves full compliance with all subject requirements, implementing 9 major modules and 4 minor modules. All 10 containers are orchestrated via Docker Compose for production deployment.
...
42 School (2024) ft_transcendence Project Requirements (v16.1). 42 School Subject Documentation.
